...
export function renderResults(result, imageSrc) {
  const { diagnosis, confidence, severity, symptoms, recommendations, cropType, analyzedAt } = result;
  ...
  document.getElementById('resultCrop').textContent = CROP_LABELS[cropType] || cropType;
  document.getElementById('resultTimestamp').textContent = _formatDate(analyzedAt);
...
export function showFilePreview(file) {
  return new Promise(resolve => {
    ...
    reader.onload = e => {
      ...
      document.getElementById('previewFilename').textContent = _truncateFilename(file.name, 32);
      resolve(src);
    };
